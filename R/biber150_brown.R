#' Distribution of Biber et al.'s (2016) 150 lexical items in the Brown Corpus (term-document matrix), using text files as corpus parts
#'
...
#' 
#' The data are provided in the form of a term-document matrix, where rows denote the 150 items and columns denote the 500 texts in the corpus. Seven items do not occur in Brown (\emph{aha, cor, cos, ltd, mhm, nought, pence)}). These are included in the term-document matrix with frequencies of 0 for all texts. Further, seven items are spelled differently in Brown (compared to the BNC, on which Biber et al.'s (2016) study is based): "u.s.a." (Brown) instead of "usa" (BNC), "inc." instead of "inc", "mr." instead of "mr", "ugh" instead of "urgh", "uh" instead of "er", "um" instead of "erm", and "hmm" instead of "hm".
#' 
#' The first row of the term-document matrix gives the length of the text (i.e. number of word and nonword tokens).
#'
#' @format ## `biber150_brown`
